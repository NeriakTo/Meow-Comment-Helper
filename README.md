...
> **行政匯入專用版 (Administrative Edition)**
> 專為國小導師設計的 AI 輔助評語撰寫系統，支援「校務系統匯入格式」與「Google 試算表雲端同步」。
...
## 📖 專案概述 (Overview)
本系統為基於 Web 的單頁應用程式 (SPA)，旨在協助導師利用生成式 AI (Gemini)，將口語觀察紀錄轉化為教育專業評語及八字箴言。**V5.1 版本針對校務行政需求進行優化**，導入 BYOB 架構保障隱私，並能產出嚴格符合學籍卡匯入標準的 Excel 檔案。
...
### 1. 建立班級 (行政版)
1. 點擊左上角的「貓窩管理」。
2. 在「建立新班級」區塊，輸入正確的 **年級** (如 4) 與 **班序** (如 7)。
3. 輸入學生人數，系統會自動產生座號。
...
### 3. 匯出檔案
1. 點擊右上角的 **「🐟 匯出匯入檔」**。
2. 系統會下載格式完全符合校務系統要求的 Excel 檔 (含正確的年級、班級代碼)。
...
## 📝 版本歷程 (Changelog)
* **V5.1 (2026-01-13)**：新增校務系統匯入格式支援、雙重語氣生成引擎 (暖心/嚴謹)、精準班級設定。
