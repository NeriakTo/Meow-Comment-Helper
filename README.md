# 🐱 喵嗚評語助手 (Meow Comment Helper) V6.0
...
## 📖 專案概述 (Overview)
本系統為基於 Web 的單頁應用程式 (SPA)，旨在協助導師利用生成式 AI (Gemini)，將口語觀察紀錄轉化為教育專業評語。**V6.0 版本導入分頁式操作介面與完整行為量表矩陣**，無論是在捷運上用手機、沙發上用平板，或是學校電腦桌前，都能輕鬆管理高達 40 個欄位的複雜學務資料，完美對接學校行政系統！
...
### 1. 系統架構 (Architecture)
V6.0 採用 **Client-Serverless + BYOB (Bring Your Own Backend)** 架構。前端負責邏輯與生成，後端資料庫由使用者自行託管於 Google Sheets。
