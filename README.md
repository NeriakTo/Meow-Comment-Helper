# 🐱 喵嗚評語助手 (Meow Comment Helper) V7.5

> **雙軌模型版 (Dual-Tier Model Edition)**
> 專為國小導師打造的療癒系評語工具——現在支援免費版與付費版雙模式，讓擁有 Paid Key 的您享受次世代 AI 的極致文采！

![Version](https://img.shields.io/badge/version-7.5-blue)
![Model](https://img.shields.io/badge/Model-Dual--Tier-purple)
...
![AI](https://img.shields.io/badge/Gemini-2.0~3.0-purple)
...
**V7.5 雙軌模型版** 帶來革命性的 **AI 分級服務**！無論您使用免費或付費 API Key，系統都會為您選配最適切的模型。付費用戶可解鎖 **Gemini 2.5 Pro / 3.0 Pro** 等先進模型，體驗細膩的文采與邏輯能力；免費用戶也能享受穩定高效的 **Gemini 2.0 Flash**。加上 V7.4 的鎖定機制與自定義標籤，這隻貓咪助手現在完全聽您的指揮！ 💎
...
這是 V7.5 的核心功能。系統現在支援 **「🌱 免費版」** 與 **「💎 付費版」** 兩種模式，根據您的 API Key 等級，自動匹配最適合的 AI 模型。

### 🎯 雙軌分流機制

| 模式 | 優先模型 | 適用對象 | 特色 |
|------|---------|---------|------|
| **🌱 免費版** | Gemini 2.0 Flash Exp | 一般使用者、教學現場 | 穩定快速、配額充足 |
| **💎 付費版** | Gemini 2.5 Pro / 3.0 Pro | 付費用戶 | 極致文采、尖端技術 |

### ✨ 模型差異說明

**免費版模型清單**：
- 🥇 `Gemini 2.0 Flash Exp` (首選) - 在免費額度內提供最佳品質與速度
- 🥈 `Gemini 1.5 Pro` (備援) - 成熟穩定的中階模型
- 🥉 `Gemini 1.5 Flash` (保底) - 速度最快、配額最寬鬆

**付費版模型清單**：
- 🏆 `Gemini 2.5 Pro` (首選) - 最強推理能力，文字細膩度遠超 2.0
- 🏆 `Gemini 3.0 Pro` (次選) - 未來最強模型 (需 Paid Tier)
- ⚡ `Gemini 2.5 Flash` (備援) - 高速且品質優異
- 🛡️ 若上述失敗，自動退守至 **免費版模型**，確保服務不中斷

### 🎨 如何切換模式

1. **頂部工具列**：在 API Key 輸入框旁，有一個精緻的下拉選單
2. **選擇等級**：
   - 🌱 **免費版 (Free)**：使用 Google 免費配額，穩定且充足
   - 💎 **付費版 (Paid)**：解鎖次世代模型，體驗極致品質
3. **自動記憶**：您的選擇會儲存在 LocalStorage，下次開啟自動恢復

### 🔄 智慧降級保護

即使您選擇了付費版，當遇到連線問題或模型暫時不可用時，系統會自動 **降級到免費模型**，確保 AI 生成服務永不中斷！

```
付費版降級路徑：
2.5 Pro → 3.0 Pro → 2.5 Flash → 2.0 Flash → 1.5 Pro → 1.5 Flash
```

> 💡 **小提醒**：若您擁有 **Paid Tier API Key**（付費方案），強烈建議切換至「💎 付費版」，體驗 Gemini 2.5/3.0 的細膩文采與邏輯推理！免費 Key 使用付費版可能會遇到配額限制。
...
V7.4.1 將「🐾 幫我寫」按鈕優化回 **懸浮於右下角** (FAB 設計)：

| 改善項目 | 說明 |
|----------|------|
| **不再遮擋** | 輸入框底部加上 `padding-bottom`，保留按鈕空間 |
| **版面平衡** | 左右兩欄高度對齊，視覺更協調 |
| **操作順手** | 按鈕固定位置，符合 Material Design 規範 |
...
| 項目 | V7.4.2 改善 |
|------|-----------|
| **逾時時間** | 從 15 秒延長至 **30 秒**，給予 API 更充裕的回應時間 |
| **連線釋放** | 無論成功或失敗，都強制釋放 `AbortController`，防止瀏覽器連線堆積 |
| **錯誤提示** | 偵測到 **429 速率限制** 時，友善提示使用者「休息 1 分鐘」 |

### 🤖 模型策略演進

- V7.4.2 曾短暫降級為保守策略（僅用 1.5 版本）
- V7.4.2 Fix 恢復高品質策略（優先 2.0 Flash Exp）
- V7.5 正式導入雙軌架構，讓免費與付費用戶都能享受最佳體驗
...
3. **✨ 餵食罐罐**：貼上 Key 即可啟動助手 (可勾選公用電腦模式 + **NEW!** 選擇金鑰等級)
...
3. **NEW!** 選擇金鑰等級（免費版 / 付費版）
4. 點擊左上角「🏡 貓窩管理」建立班級
5. 選擇學生、輸入觀察紀錄、點擊「🐾 幫我寫」
...
2. 選擇學生，輸入姓名與觀察紀錄
3. **NEW!** 切換金鑰等級（若有 Paid Key，強烈建議選「💎 付費版」）
4. 點擊「🐾 幫我寫」生成評語
5. **V7.4** 若滿意評語但想改箴言，點擊 🔓 鎖定評語後再按一次「幫我寫」
...
| **AI 引擎** | Google Gemini API (支援 1.5 / 2.0 / 2.5 / 3.0) |
...
### V7.5 (2026-01-16) - 雙軌模型版 💎

**💎 雙軌架構**
- 新增 **API Key 等級選擇**：支援「🌱 免費版」與「💎 付費版」雙模式
- **付費版優先模型**：`Gemini 2.5 Pro`、`Gemini 3.0 Pro`、`Gemini 2.5 Flash`
- **免費版穩定模型**：`Gemini 2.0 Flash Exp`、`Gemini 1.5 Pro`、`Gemini 1.5 Flash`
- **智慧降級**：付費模型失敗時自動退守至免費模型，確保服務不中斷

**⚙️ UI 新增**
- 頂部工具列新增 **金鑰等級下拉選單**，付費版使用紫金漸層配色彰顯尊榮
- 新手引導精靈 (步驟 3) 同步新增金鑰等級選擇，初次設定即可選對

**🔧 持久化機制**
- 金鑰等級選擇自動儲存至 LocalStorage (`gemini_key_tier`)
- 下次開啟自動恢復，無需重複設定

---

### V7.4.2 (2026-01-16) - 穩定性強化 ⚡

**⏱️ 連線與逾時優化**
- 將 API 逾時時間從 15 秒延長至 **30 秒**
- 新增 **強制連線釋放**：無論成功失敗，都執行 `controller.abort()`，防止瀏覽器連線堆積

**🛡️ 錯誤處理強化**
- 新增 **429 速率限制偵測**：偵測到「資源耗盡」或「429」錯誤時，友善提示「休息 1 分鐘」
- 立即中止重試機制，避免越試越鎖

**🤖 模型策略修正**
- 初版降級為保守策略 (僅用 1.5 穩定版)
- 隨後修正為品質優先策略 (恢復 2.0 Flash Exp)

---

### V7.4.1 (2026-01-16) - UI/UX 美感修復 🎨

**📐 版面佈局重構**
- 左右兩欄設定為 `flex-col`，觀察紀錄與老師評語區塊使用 `flex-1`
- 桌面版左右欄底部完美對齊，視覺平衡感大幅提升

**🎯 懸浮按鈕優化**
- 將「🐾 幫我寫」按鈕從獨立 Action Bar 恢復至 **右下角懸浮** (FAB)
- 輸入框新增 `pb-20` 防止文字被按鈕遮擋

**🏷️ 學籍卡評語標題美化**
- 鎖定按鈕與灰色標籤並排於右上角，版面更精緻
...
**📐 UI 優化**
- 新增底部 **獨立操作列 (Action Bar)**：「🐾 幫我寫」按鈕不再遮擋文字內容 (V7.4.1 改回懸浮式)
- 優化輸入框與操作區的視覺層次與間距
...
<p align="center">
  <strong>🐾 喵嗚評語助手 V7.5 - 溫暖、安全、智慧分級，讓期末評語變得輕鬆療癒 💎</strong>
</p>
